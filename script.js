...
// ── API ──
// Apps Script does not support CORS headers on POST responses.
// All mutations go through GET with a ?payload= param — zero CORS issues, no preflight.
...
function initMakePage() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const xHandle  = document.getElementById('f-x').value.trim();
    ...
    if (!xHandle) { setFieldError('f-x',       'X handle is required so supporters can reach you'); hasError = true; }
    if (!wallet)  {
    ...
    try {
      ...
      const base        = location.href.split('?')[0].replace('make.html', 'confirm.html');
      const confirmUrl  = base + '?token=' + result.claim_token;

      form.style.display      = 'none';
      successEl.style.display = 'block';
      document.getElementById('confirm-link').textContent = confirmUrl;
      document.getElementById('copy-btn').onclick = () => copyText(confirmUrl, document.getElementById('copy-btn'));
...
function initFulfillPage() {
  ...
  function buildWishCard(w) {
    ...
    div.querySelector('.support-btn').addEventListener('click', async (e) => {
      ...
      try {
        ...
        `;
        actionEl.querySelector('.copy-wallet-btn').addEventListener('click', ce => copyText(w.wallet, ce.currentTarget));
  ...
  if (devCard) {
    ...
    if (copyWalletBtn) {
      copyWalletBtn.addEventListener('click', () => copyText('0x00547C5811c09102cB632Bbb945C395F8567d573', copyWalletBtn));
    }
...
async function initConfirmPage() {
  ...
  try {
    ...
    if (wish.status === 'completed') {
      ...
      result.style.cssText = 'display:block;color:#357a55;margin-top:1rem;text-align:center;';
      return;
    }
    ...
    if (wish.status === 'open') {
      ...
      result.style.cssText = 'display:block;color:var(--ink-faint);margin-top:1rem;text-align:center;';
    }
    ...
    btnYes.addEventListener('click', async () => {
      ...
      try {
        ...
        result.style.cssText = 'display:block;color:#c0624a;margin-top:1rem;text-align:center;';
      }
    });
